...
Class Master extends DBConnection {
	...
	function place_order(){
		...
		if(!$client_id){
			return json_encode(['status' => 'failed', 'msg' => 'No client ID found. Please log in again.']);
		}
	
		$cart_items = $this->conn->query("SELECT * FROM `cart_list` where client_id ='{$client_id}'");
		if(!$cart_items){
			error_log("Cart items query failed: " . $this->conn->error);
			return json_encode(['status' => 'failed', 'msg' => 'Failed to retrieve cart items.']);
		}
	
		if($cart_items->num_rows <= 0){
			return json_encode(['status' => 'failed', 'msg' => 'There are no items in the cart.']);
		}
		error_log("Starting place_order function for user ID: " . $this->settings->userdata('id'));
	
		$inserted = [];
		$has_failed = false;
		$gtotal = 0; // Total amount including vouchers
		$price_before_voucher = 0; // Price before applying vouchers
		...
		if($vendors === false){
			error_log("Failed to retrieve vendors: " . $this->conn->error);
			return json_encode(['status' => 'failed', 'msg' => 'Failed to retrieve vendors']);
		}
		...
		while($vrow = $vendors->fetch_assoc()):
			...
			error_log("Generated order reference code: " . $ref_code);
			...
			if($save){
				...
				$inserted[] = $oid;
				$data = "";
				$gtotal = 0; // Reset total for each vendor
				...
				if($products === false){
					error_log("Failed to retrieve products: " . $this->conn->error);
					$has_failed = true;
					break;
				}
				...
				while($prow = $products->fetch_assoc()):
					...
					$price_before_voucher += $total; // Accumulate price before vouchers
					// If there is a voucher applied, calculate the total after vouchers
					if (isset($_SESSION['discounted_total'])) {
						$gtotal = $_SESSION['discounted_total'];
						// Update 'final_total' with the discounted total
						$this->conn->query("UPDATE `order_list` SET `final_total` = '{$gtotal}' WHERE `id` = '{$oid}'");
					} else {
						// If no voucher, do nothing for 'final_total'
					}
					
					// Always update 'total_amount' with the original total
				...
				if($save2){
					error_log("Order items saved successfully for order ID: " . $oid);
				} else {
					$has_failed = true;
					$resp['sql'] = $sql2;
					error_log("Failed to insert order items: " . $sql2);
					break;
				}
			} else {
				$has_failed = true;
				$resp['sql'] = $sql;
				error_log("Failed to insert order: " . $sql);
				break;
			}
		endwhile;
		...
		if(!$has_failed){
			...
			// Update the 'total_amount' and 'price_before_voucher' columns in the order_list table
			$this->conn->query("UPDATE `order_list` SET `total_amount` = '{$gtotal}', `final_total` = '{$gtotal}' WHERE `id` = '{$oid}'");
			$this->conn->query("DELETE FROM `cart_list` where client_id ='{$this->settings->userdata('id')}'");
			unset($_SESSION['discounted_total']);
			error_log("Order placed successfully for user ID: " . $this->settings->userdata('id'));
		} else {
			$resp['status'] = 'failed';
			...
			if(count($inserted) > 0){
				error_log("Deleting partially completed order due to failure for orders: " . implode(',', array_map('intval', $inserted)));
				$this->conn->query("DELETE FROM `order_list` where id in (".implode(',', array_map('intval', $inserted)).") ");
			}
		}
		...
		if($resp['status'] == 'success'){
			...
			error_log("Order placement failed with message: " . $resp['msg']);
		}
	...
	function get_cart_items($client_id) {
	...
	function apply_voucher(){
		...
		$voucherQuery = $this->conn->query("SELECT * FROM `vouchers` WHERE `voucher_code` = '{$voucher_code}' AND `expiry_date` >= CURDATE() AND `is_used` < `usage_limit` LIMIT 1");
	
		if ($voucherQuery->num_rows > 0) {
			$voucherData = $voucherQuery->fetch_assoc();
	
			$clientID = $this->settings->userdata('id');
			$cartTotalQuery = $this->conn->query("SELECT SUM(quantity * price) AS total FROM `cart_list` c INNER JOIN product_list p ON c.product_id = p.id WHERE c.client_id = '{$clientID}'");
	
			if($cartTotalQuery){
				$cartTotalResult = $cartTotalQuery->fetch_assoc();
				$cartTotal = $cartTotalResult['total'];
	
				$discountAmount = ($cartTotal * ($voucherData['discount_percentage'] / 100.00));
				$updatedTotal = $cartTotal - $discountAmount;
	
				// Store both price_before_voucher and price_after_voucher in the session
				$_SESSION['price_before_voucher'] = $cartTotal;
				$_SESSION['price_after_voucher'] = $updatedTotal;
	
				$updateVoucherUsage = $this->conn->query("UPDATE `vouchers` SET `is_used` = `is_used` + 1 WHERE `voucher_code` = '{$voucher_code}'");
	
				if($updateVoucherUsage){
					$resp['status'] = 'success';
					$resp['msg'] = 'Voucher applied successfully!';
					$resp['updated_total'] = $updatedTotal;
				} else {
					$resp['status'] = 'failed';
					$resp['msg'] = 'Failed to update the voucher usage.';
					$resp['error'] = $this->conn->error;
				}
			} else {
				$resp['status'] = 'failed';
				$resp['msg'] = 'There was a problem calculating the cart total.';
				$resp['error'] = $this->conn->error;
			}
		} else {
			$resp['status'] = 'failed';
			$resp['msg'] = 'Invalid voucher code, voucher has expired, or usage limit reached!';
		}
