...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    if ($qry->num_rows > 0) {
        ...
        $discount_html = isset($_GET['discount']) ? urldecode($_GET['discount']) : '';

        // Check if the product is 100% Egyptian
        $is_100_percent_egyptian = isset($is_100_percent_egyptian) ? $is_100_percent_egyptian : 0;
    } else {
...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    if ($qry->num_rows > 0) {
        ...
        // Get the discount parameter from the URL and validate it
        $discount_param = isset($_GET['discount']) ? urldecode($_GET['discount']) : '';
        $discount_html = filter_var($discount_param, FILTER_SANITIZE_STRING);
    } else {
        echo "<script> alert('Unknown Product ID.'); location.replace('./?page=products') </script>";
        exit;
    }
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-8 col-md-7 col-sm-12">
                        ...
                        <!-- Display Expiry Date -->
                        <div class="d-flex">
                            <div class="col-auto px-0"><small class="text-muted">Expiry Date: </small></div>
                            <div class="col-auto px-0 flex-shrink-1 flex-grow-1"> <p class="m-0"><small class="text-muted"><?= $expiry_date ?></small></p></div>
                        </div>
                        <div class="d-flex">
                            <div class="col-auto px-0"><small class="text-muted">Discount: </small></div>
                            <div class="col-auto px-0 flex-shrink-1 flex-grow-1">
                                <p class="m-0">
                                    <small class="text-danger"><?= $discount_html ?></small>
                                </p>
                            </div>
                        </div>
                        <div class="d-flex">
                            <div class="col-auto px-0"><small class="text-muted">100% Egyptian Product: </small></div>
                            <div class="col-auto px-0 flex-shrink-1 flex-grow-1">
                                <?php if ($is_100_percent_egyptian == 1) : ?>
                                    <p class="m-0">
                                        <span class="badge badge-egyptian">100% Egyptian</span>
                                    </p>
                                <?php else : ?>
                                    <p class="m-0">
                                        <span class="badge badge-secondary">Not 100% Egyptian</span>
                                    </p>
                                <?php endif; ?>
                            </div>
                        </div>
